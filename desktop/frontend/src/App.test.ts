...
describe('App', () => {
  ...
  it('switches to the Operations Log view when its sidebar item is clicked', async () => {
    render(App);
    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'Scrub & Move' })).toBeInTheDocument();
    });
  ...
  it('switches to Scrub & Move and back to Library when their sidebar items are clicked', async () => {
    render(App);
    await waitFor(() => {
      expect(screen.getByText('No books found in the library folder yet.')).toBeInTheDocument();
    });

    await fireEvent.click(screen.getByRole('button', { name: 'Scrub & Move' }));
    await waitFor(() => {
