...
- Config `fallbacks.year` / `fallbacks.author` text is used **only** when rendering a destination-path preview for a field that is `Unresolved` — it does not change that field's `Source`.
- **Post-completion behavior change (2026-07-10):** `Book.Status()` originally treated any single `Unresolved` field (Title, Author, or Year) as making the whole row `Unresolved` and excluded from Apply. User-requested change: Title is the only field Apply cannot proceed without (found via manual testing against real files where several otherwise-good books were being skipped purely because Author or Year couldn't be parsed from the filename). `Book.Status()` now returns `SourceUnresolved` only when Title itself is unresolved; if Title is resolved but Author and/or Year are not, it returns the new `SourcePartial` — still eligible for Apply (the rendered `DestPath` uses the configured fallback text for the missing field), just flagged as incomplete for the user's awareness.
