...
- Every field-level resolution in a `book.Book` carries a `Source` (`Metadata` / `Heuristic` / `Manual` / `Unresolved` / `Partial`). Manual is set later by the UI layer (not this plan) when a user edits a field — this plan's code must leave room for it (the `Source` enum includes it) but nothing in this plan ever produces `SourceManual` itself. `Partial` is row-level only (`Book.Status()`), never a valid `Field.Source`.
- **Post-completion behavior change (2026-07-10):** `Book.Status()` originally treated any single `Unresolved` field (Title, Author, or Year) as making the whole row `Unresolved` and excluded from Apply. User-requested change: Title is the only field Apply cannot proceed without (found via manual testing against real files where several otherwise-good books were being skipped purely because Author or Year couldn't be parsed from the filename). `Book.Status()` now returns `SourceUnresolved` only when Title itself is unresolved; if Title is resolved but Author and/or Year are not, it returns the new `SourcePartial` — still eligible for Apply, just flagged as incomplete for the user's awareness.
- **Post-completion behavior change (2026-07-10):** the config's `fallbacks.year`/`fallbacks.author` settings (and `config.Fallbacks` / `cfg.General.Fallbacks`) have been **removed entirely**. An unresolved Author and/or Year is no longer rendered as placeholder text ("Unknown Author") in the destination filename — `rename.BuildPath`/`render` now omits that field's decorative template segment (the `" - {author}"` suffix or the `"({year})"` parenthetical) entirely, e.g. `"Build Your API with Spring.pdf"` instead of `"Build Your API with Spring (2022) - Unknown Author.pdf"` when only Author is missing. This is a best-effort cleanup pass (empty-parens removal, whitespace collapse, leading/trailing separator trim) tuned for the default `"{title} ({year}) - {author}"` template shape and most conventional custom templates, not a general templating engine.
